...
<script>
import { getRoleList, getCompanyInfo, deleteRole, updateRole, getRoleDetail, addRole, assignPrem } from '@/api/setting'
...
export default {
  ...
  computed: {
    ...
    showTitle() {
      return this.roleForm.id ? '编辑角色' : '删除角色'
    }
  },
  created() {
  ...
  methods: {
    ...
    async btnOK() {
      try {
        ...
        this.showDialog = false // 关闭弹层 ==> 触发el-dialog的@close事件
      } catch (error) {
        console.log(error)
      }
    },
